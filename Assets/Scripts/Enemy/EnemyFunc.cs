...
{
    public float HpP_Plus = 0f;
    public float HpC_Plus = 0f;
    ...
    /// <param name="g"></param>
    /// <returns></returns>
    public IEnumerator EnemyCreateFunc1(int cnt, float sMin, float sMax, int cMin, int cMax,float Hpc,float HpP, params string[] g)
    {
        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            GameObject enemy = ObjectPooler.SpawnFromPool(g[r], GetRandomPos(), Quaternion.LookRotation(MainSingleton.instance.Player.transform.position));
                enemy.GetComponent<Enemy>().CreateStart(Hpc + HpC_Plus, HpP + HpP_Plus);
            }
    ...
    /// <param name="g"></param>
    /// <returns></returns>
    public IEnumerator EnemyCreateFunc2(int cnt, float Hpc, float HpP, params string[] g)
    {
        if (MainSingleton.instance.playerinfo.Clear)
        {
            yield break;
    ...
    public IEnumerator EnemyCreateFuncCircle(string g,float Hpc, float HpP)
    {
        if (MainSingleton.instance.playerinfo.Clear)
        {
            yield break;
        ...
        for (int i = 0; i < 36; i++)
        {
            ...
            bat.GetComponent<Enemy>().CreateStart(Hpc + HpC_Plus, HpP + HpP_Plus);
            //플레이어 위치에서 원형으로 생성
            //bat.transform.position = playerPos + circlePos;
        }
    }

    public IEnumerator EnemyCreateFuncBat(int cnt,float min,float max,string g,float Hpc, float HpP)
    {
        if (MainSingleton.instance.playerinfo.Clear)
        {
            yield break;
        ...
        Vector3 batPos = GetRandomPos() * 1.3f;
            for (int i = 0; i < 20; i++)
            {
                ...
                bat.GetComponent<EnemyBat>().CreateStart(Hpc + HpC_Plus, HpP + HpP_Plus);
            }
    ...
    public IEnumerator BossCreate(string g,float Hpc, float HpP, Vector3 pos)
    {
        if (MainSingleton.instance.playerinfo.Clear)
        {
            yield break;
