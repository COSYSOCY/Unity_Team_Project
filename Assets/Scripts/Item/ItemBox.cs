...
public class ItemBox : MonoBehaviour
{
    ...
    public void Update()
    {
        LevelUiObject = GameObject.Find("Canvas").transform.Find("LevelUi").gameObject;
        skillbox = GameObject.Find("SkillManager");
        skillManager = skillbox.GetComponent<SkillManager>();
        Plmove = GameObject.Find("ControlPanel");
        moving = Plmove.GetComponent<PlayerMoving>();
    }
    ...
    public void BoxFunc()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (Lv == 0)
            {
                Skill_Lv[i].GetComponent<Text>().text = "Ω≈±‘!";
            }
            else
            {
                Skill_Lv[i].GetComponent<Text>().text = "∑π∫ß: " + (Lv + 1);
            }
